...
namespace Nanami
{
    internal class PlayerPvpData
    {
        ...
        public PlayerPvpData(int index) { PlayerIndex = index; }

        public static PlayerPvpData GetPlayerData(int index)
            => GetPlayerData(TShock.Players[index]);
        public static PlayerPvpData GetPlayerData(TSPlayer player)
            => player?.GetData<PlayerPvpData>(Key);
        ...
        {
            Deaths++;
            Endurance -= (int)Main.CalculateDamagePlayersTake(dmg, Main.player[PlayerIndex].statDefense);
            if (KillStreak >= Nanami.Config.MinKillTime)
        ...
        public void Hurt(int calculatedDmg) => Endurance += calculatedDmg;

        /// <summary>玩家攻击事件</summary>
        /// <param name="calculatedDmg">经计算的攻击数值</param>
        public void Damage(int calculatedDmg) => DamageDone += calculatedDmg;

        public static PlayerPvpData LoadFromDb(int index, QueryResult reader) => new PlayerPvpData(index)
        {
